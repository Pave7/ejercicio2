...
class Attendee(models.Model):
    ...
    def _send_mail_to_attendees(self, template_xmlid, force_send=False):
        ...
        calendar_view = self.env.ref('calendar_sitt.view_calendar_event_calendar')
    ...
    def do_accept(self):
        ...
        for attendee in self:
            attendee.event_id.message_post(body=_("%s has accepted invitation") % (attendee.common_name), subtype="calendar_sitt.subtype_invitation")
        return result
    ...
    def do_decline(self):
        ...
        for attendee in self:
            attendee.event_id.message_post(body=_("%s has declined invitation") % (attendee.common_name), subtype="calendar_sitt.subtype_invitation")
        return res
...
class Meeting(models.Model):
    ...
    def create_attendees(self):
        ...
        for meeting in self:
            ...
            if meeting_attendees:
                ...
                to_notify._send_mail_to_attendees('calendar_sitt.calendar_template_meeting_invitation')
    ...
    def action_sendmail(self):
        ...
        if email:
            for meeting in self:
                meeting.attendee_ids._send_mail_to_attendees('calendar_sitt.calendar_template_meeting_invitation')
        return True
            ...
            if (values.get('start_date') or values.get('start_datetime') or
                ...
                for current_meeting in all_meetings:
                    ...
                    if attendee_to_email:
                        attendee_to_email._send_mail_to_attendees('calendar_sitt.calendar_template_meeting_changedate')
        return True
